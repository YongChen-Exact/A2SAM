...
from utils_.pruning import  prune_by_percentile_gradient_perCell, calculate_contrastive_score
import wandb
import socket
...
def eval_psnr(loader, model, eval_type=None):
    ...
    for batch in loader:
        ...
        #     batch[k] = v.cuda()
        pred = torch.sigmoid(model.infer(batch['inp'], batch['unc'], batch['lgt']))
...
def train(train_loader, model, mask, tuning_mode):
    ...
    for batch in train_loader:
        ...
        gt = batch['gt']
        unc_map = batch['unc']
        coarse_logit = batch['lgt']
        model.set_input(inp, gt, unc_map, coarse_logit)
...
def main(config_, save_path, args):
    ...
    if args.tuning_mode == "gps" or args.tuning_mode == "cps":
    ...
    net_new_dict = load_from(model.state_dict(), sam_checkpoint, 1024, 16)

    model.load_state_dict(net_new_dict, strict=False)
    
    for name, para in model.named_parameters():
        if args.tuning_mode == "gps" or args.tuning_mode == "full" or args.tuning_mode == "cps":
            pass
    ...
    if args.tuning_mode == "cps":
        calculate_gradient(model, val_loader_grad)
        mask = calculate_contrastive_score(log, model)
    elif args.tuning_mode == "gps":
        calculate_gradient(model, val_loader_grad)
        mask = prune_by_percentile_gradient_perCell(log, model, args.times_para)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tag', default=None,help="gps or cps")
    parser.add_argument("--local_rank", type=int, default=0, help="")
    parser.add_argument('--nproc_per_node', type=int, default=1, help='number of processes per node')
    parser.add_argument('--nnodes', type=int, default=1, help='number of nodes')
    parser.add_argument('--tuning-mode', default='cps', type=str, help='gps cps')
